@extends('template_category.home')
@section('content') 


<link href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js"></script>
<script src="https://code.jquery.com/jquery-1.11.1.min.js"></script>
<link rel="stylesheet" href="{{ asset('assets/css/sana.css')}}" />
<!------ Include the above in your HEAD tag ---------->

<!-----------------------------------Section first--------------------------------------->
<link href='https://fonts.googleapis.com/css?family=Roboto' rel='stylesheet'>
<section>
	<div class="container tb-padding">
		<div class="row">
			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
				
				<a href="about.html" class="btn_orange medium cover-size text-center">VIEW ON MAP</a>
				<div class="side-bar">
					<ul>
						<li><i class="fa fa-wrench" aria-hidden="true"></i>RELATED SERVICE PROVIDERS</li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Electricians</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Plumbers</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Carpenters</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Hospitals</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Ambulance Services</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Blood Banks</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Wallpapers Dealers</a></li>
					</ul>
				</div>
				<div class="side-bar">
					<ul>
						<li><i class="fa fa-paper-plane-o" aria-hidden="true"></i>Locations Service Providers</li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">New Delhi</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Amritsar</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Chandigarh</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Mumbai</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Jaipur</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Mohali</a></li>
						<li><i class="fa fa-angle-double-right" aria-hidden="true"></i><a href="">Banglore</a></li>
					</ul>
				</div>
				<div class="need-help text-center">
					<i class="fa fa-user" aria-hidden="true"></i>
					<h5>Need Help?</h5>
					<h4>+45-455-453</h4>
					<p><span>Monday to Friday 9.00am - 7.30pm</span></p>
				</div>
			</div>
		
			
			<div class="col-lg-8 col-md-8 col-sm-8 col-xs-12">
				<div class="search-box">
					<div class="row">
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
						<select>
							<option>Sort By Price</option>
							<option>Sort By Price</option>
							<option>Sort By Price</option>
						</select>
						</div>
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
						<select>
							<option>Sort By Ranking</option>
							<option>Sort By Ranking</option>
							<option>Sort By Ranking</option>
						</select>
						</div>
						<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
							<i class="fa fa-list" aria-hidden="true"></i>
							<i class="fa fa-th" aria-hidden="true"></i>
						</div>
				</div>
			</div>
			
			<div class="row small-padding-bg">
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<div class="image-size"><img src="https://images.pexels.com/photos/224924/pexels-photo-224924.jpeg?w=940&h=650&auto=compress&cs=tinysrgb"/></div>
				</div>
				<div class="col-lg-5 col-md-5 col-sm-5 col-xs-5 customs-p">
					<h3>Maha Laxmi Break Service</h3>
					<p><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><span>233 Votes</span></p>
					<p><span><i class="fa fa-mobile" aria-hidden="true"></i></span>+91-9945899688</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>Dwarka Sector 11 New Delhi</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>24X7 Service Providers....</p>
				</div>
				<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
					<a href="about.html" class="btn_orange medium customs-margin">Read more</a>
				</div>
			</div>
			<div class="row small-padding-bg">
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<div class="image-size"><img src="https://images.pexels.com/photos/224924/pexels-photo-224924.jpeg?w=940&h=650&auto=compress&cs=tinysrgb"/></div>
				</div>
				<div class="col-lg-5 col-md-5 col-sm-5 col-xs-5 customs-p">
					<h3>Maha Laxmi Break Service</h3>
					<p><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><span>233 Votes</span></p>
					<p><span><i class="fa fa-mobile" aria-hidden="true"></i></span>+91-9945899688</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>Dwarka Sector 11 New Delhi</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>24X7 Service Providers....</p>
				</div>
				<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
					<a href="about.html" class="btn_orange medium customs-margin">Read more</a>
				</div>
			</div>
			
				
			
			
			<div class="row small-padding-bg">
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<div class="image-size"><img src="https://images.pexels.com/photos/326410/pexels-photo-326410.jpeg?h=350&auto=compress&cs=tinysrgb"/></div>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 customs-p">
					<h4>Maha Laxmi Break Service</h4>
					<p><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><span>233 Votes</span></p>
					<p><span><i class="fa fa-mobile" aria-hidden="true"></i></span>+91-9945899688</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>Dwarka Sector 11 New Delhi</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>24X7 Service Providers....</p>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<button class="btn btn-danger customs-margin">Best Deals </button>
				</div>
			</div>
			
			<div class="row small-padding-bg">
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<div class="image-size"><img src="https://images.pexels.com/photos/326410/pexels-photo-326410.jpeg?h=350&auto=compress&cs=tinysrgb"/></div>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 customs-p">
					<h4>Maha Laxmi Break Service</h4>
					<p><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><span>233 Votes</span></p>
					<p><span><i class="fa fa-mobile" aria-hidden="true"></i></span>+91-9945899688</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>Dwarka Sector 11 New Delhi</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>24X7 Service Providers....</p>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<button class="btn btn-danger customs-margin">Best Deals </button>
				</div>
			</div>
			
			<div class="row small-padding-bg">
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<div class="image-size"><img src="https://images.pexels.com/photos/326410/pexels-photo-326410.jpeg?h=350&auto=compress&cs=tinysrgb"/></div>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 customs-p">
					<h4>Maha Laxmi Break Service</h4>
					<p><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><span>233 Votes</span></p>
					<p><span><i class="fa fa-mobile" aria-hidden="true"></i></span>+91-9945899688</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>Dwarka Sector 11 New Delhi</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>24X7 Service Providers....</p>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<button class="btn btn-danger customs-margin">Best Deals </button>
				</div>
			</div>
			
			<div class="row small-padding-bg">
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<div class="image-size"><img src="https://images.pexels.com/photos/326410/pexels-photo-326410.jpeg?h=350&auto=compress&cs=tinysrgb"/></div>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 customs-p">
					<h4>Maha Laxmi Break Service</h4>
					<p><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><span>233 Votes</span></p>
					<p><span><i class="fa fa-mobile" aria-hidden="true"></i></span>+91-9945899688</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>Dwarka Sector 11 New Delhi</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>24X7 Service Providers....</p>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<button class="btn btn-danger customs-margin">Best Deals </button>
				</div>
			</div>
			
			<div class="row small-padding-bg">
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<div class="image-size"><img src="https://images.pexels.com/photos/326410/pexels-photo-326410.jpeg?h=350&auto=compress&cs=tinysrgb"/></div>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 customs-p">
					<h4>Maha Laxmi Break Service</h4>
					<p><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><i class="fa fa-star-o" aria-hidden="true"></i><span>233 Votes</span></p>
					<p><span><i class="fa fa-mobile" aria-hidden="true"></i></span>+91-9945899688</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>Dwarka Sector 11 New Delhi</p>
					<p><span><i class="fa fa-location-arrow" aria-hidden="true"></i></span>24X7 Service Providers....</p>
				</div>
				<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
					<button class="btn btn-danger customs-margin">Best Deals </button>
				</div>
			</div>
				<ul class="pagination">
					<li class="page-item"><a class="page-link" href="#">Previous</a></li>
					<li class="page-item"><a class="page-link" href="#">1</a></li>
					<li class="page-item"><a class="page-link" href="#">2</a></li>
					<li class="page-item"><a class="page-link" href="#">3</a></li>
					<li class="page-item"><a class="page-link" href="#">4</a></li>
					<li class="page-item"><a class="page-link" href="#">5</a></li>
					<li class="page-item"><a class="page-link" href="#">6</a></li>
					<li class="page-item"><a class="page-link" href="#">7</a></li>
					<li class="page-item"><a class="page-link" href="#">8</a></li>
					<li class="page-item"><a class="page-link" href="#">9</a></li>
					<li class="page-item"><a class="page-link" href="#">Next</a></li>
				  </ul>
		</div>
	</div>
	</div>
</section>



@endsection
	
					

